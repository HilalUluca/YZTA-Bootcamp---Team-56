...
def seed_tasks(db, user: User, rng: random.Random, days: int, today: date) -> dict[str, int]:
    """Gün gün görev geçmişi üretir."""
...
def seed_focus_sessions(
    db, user: User, rng: random.Random, days: int, today: date
) -> tuple[int, int]:
    """
    Odaklanma seansları üretir. Farklı saatler ve farklı verimlilik puanları;
    verimlilik zaman içinde hafifçe yükselir (kullanıcı toparlanıyor hikayesi).
...
def seed_habits(db, user: User, rng: random.Random, today: date) -> tuple[int, int]:
    """
    ...
    specs = [
        {
            ...
            "target_value": 30,
            "unit": "sayfa",
            # Son 9 gün kesintisiz
            "log_offsets": list(range(1, 10)),
        },
        {
            ...
            "target_value": 2,
            "unit": "litre",
            # Son 14 gün, biri atlanmış
            "log_offsets": [d for d in range(1, 15) if d != 6],
        },
        {
    ...
    for spec in specs:
        ...
        habit.last_completed_at = utc(max(log_days), 20) if log_days else None

    db.commit()
    return len(specs), total_logs


def seed_device_usage(db, user: User, rng: random.Random, today: date, days: int = 14) -> int:
    """
...
def main() -> int:
    ...
    parser.add_argument("--full-name", default="Demo Kullanıcı")
    ...
    try:
        ...
        task_stats = seed_tasks(db, user, rng, args.days, today)
        ...
        session_count, focus_xp = seed_focus_sessions(db, user, rng, args.days, today)
